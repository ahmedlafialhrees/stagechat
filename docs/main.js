// تحميل THREE ديناميكياً مع بديل
let THREE;
try {
...
if (!THREE) { alert("تعذّر تحميل Three.js"); throw new Error("THREE failed"); }
...
const { WebGLRenderer, Scene, Fog, PerspectiveCamera, HemisphereLight, DirectionalLight,
        ...
        CylinderGeometry, PointLight, SphereGeometry, CapsuleGeometry } = THREE;
...
const hemi = new HemisphereLight(0x66aaff, 0x0b0f18, 0.6);
scene.add(hemi);
...
const bMat = new MeshStandardMaterial({color:0x0b6cff, emissive:0x072c6f, emissiveIntensity: 0.25, metalness:0.6, roughness:0.35});
...
for (let x=-9; x<=9; x++){
  for (let z=-9; z<=9; z++){
    ...
    city.add(m);
    if (Math.random()<0.25){
      const line = new Mesh(new BoxGeometry(2.02, 0.05, 0.1), glowMat);
...
const body = new Mesh(new BoxGeometry(0.35,0.22,0.8), new MeshStandardMaterial({color:0x222831, metalness:0.9, roughness:0.2}));
body.position.set(0.3,-0.21,-0.7);
const barrel = new Mesh(new CylinderGeometry(0.05,0.05,0.6,16), new MeshStandardMaterial({color:0x2d89ef, emissive:0x1133aa, emissiveIntensity:0.5, metalness:0.8, roughness:0.25}));
...
const eMat = new MeshStandardMaterial({color:0xff5a5a, emissive:0x4e0202, emissiveIntensity: 0.3, metalness:0.2, roughness:0.9});
function spawnEnemy(){
  const angle = Math.random()*Math.PI*2, radius = 45 + Math.random()*10;
  const m = new Mesh(eGeo, eMat);
  m.castShadow = true; m.receiveShadow = true;
  m.position.set(Math.cos(angle)*radius, 0.9, Math.sin(angle)*radius);
...
addEventListener('touchmove', e=>{
  for(const t of e.changedTouches){
    const x=t.clientX,y=t.clientY;
    if(t.identifier===moveTouchId){ const dx=(x-left.x)/60, dy=(y-left.y)/60; left.dx=Math.max(-1,Math.min(1,dx)); left.dy=Math.max(-1,Math.min(1,dy)); setStick(stickLeft,left.dx,left.dy); }
    else if(t.identifier===lookTouchId){ const dx=(x-right.x)/2.5, dy=(y-right.y)/2.5; player.yaw-=dx*0.01; player.pitch-=dy*0.01; player.pitch=Math.max(-1.2,Math.min(1.2,player.pitch)); right.x=x; right.y=y; setStick(stickRight,0,0); }
  }
},{passive:true});
...
/* حلقة اللعبة */
let shootPressed=false, running=false, last=performance.now()/1000;
function reset(){
...
function updateEnemies(dt){
  for(const e of enemies){
    const to=new THREE.Vector3().subVectors(player.pos,e.position); to.y=0;
    ...
    for(const e of enemies){
      ...
      if(e.position.distanceToSquared(b.position)<0.7*0.7){
        ...
        if(e.userData.hp<=0){ e.userData.dead=true; score+=10; scoreEl.textContent=score;
          const pl=new PointLight(0xff7766,2.5,6,2); pl.position.copy(e.position).add(new THREE.Vector3(0,1,0)); scene.add(pl); setTimeout(()=>scene.remove(pl),80);
        }
      }
    }
  }
...
function step(ms){
  if(!running) return;
  let dt=Math.min(0.033, ms/1000 - last); last=ms/1000;
