// لا نستخدم top-level await (لسفاري). نمنع كل سلوكيات الصفحة اللي تسبب نسخ/تكبير/سحب.
// نحاول قفل الاتجاه عند البدء (قد يرفضه المتصفح).

(function () {
  // منع نسخ/قائمة مطوّلة/تكبير/سحب/دبل-تاب
  const block = e => e.preventDefault();
  ['contextmenu','copy','cut','paste','selectstart','gesturestart','dblclick'].forEach(ev=>{
    document.addEventListener(ev, block, {passive:false});
  });
  window.addEventListener('scroll', ()=> scrollTo(0,0));
  // سنمنع لمس على الكانفاس لاحقاً أيضاً

  // تحميل THREE ديناميكياً من مصدرين
  function loadThree(){
    return import('https://cdn.jsdelivr.net/npm/three@0.161.0/build/three.module.js')
      .catch(()=>import('https://unpkg.com/three@0.161.0/build/three.module.js'));
  }
  ...
  async function start() {
    ...
    const {
      ...
      HemisphereLight, DirectionalLight, PlaneGeometry, MeshStandardMaterial, Mesh,
      BoxGeometry, MeshBasicMaterial, CylinderGeometry, PointLight, SphereGeometry, CapsuleGeometry,
      Vector3, Euler, ACESFilmicToneMapping, sRGBEncoding
    } = THREE;
    ...
    // تطبيق الحظر على الكانفاس
    canvas.addEventListener('touchmove', block, {passive:false});

    // Renderer واقعية خفيفة
    ...
    const hemi = new HemisphereLight(0x8fc1ff, 0x0b0f18, 0.75); scene.add(hemi);
    ...
    // أرض + مدينة نيون خيالية
    const ground = new Mesh(new PlaneGeometry(420, 420), new MeshStandardMaterial({color:0x101826, metalness:0.1, roughness:1}));
    ground.rotation.x = -Math.PI/2; ground.receiveShadow = true; scene.add(ground);

    const city = new THREE.Group();
    const bMat   = new MeshStandardMaterial({color:0x0b6cff, emissive:0x072c6f, emissiveIntensity:.3, metalness:.55, roughness:.35});
    ...
    for (let x=-9; x<=9; x++) for (let z=-9; z<=9; z++) {
      ...
      if (Math.random()<.3){
        const line=new Mesh(new BoxGeometry(2.02,.05,.1), glowMat);
        line.position.set(m.position.x, Math.random()*h, m.position.z+1.05); city.add(line);
      }
    }
    scene.add(city);

    // لاعب + "سلاح" مبسّط شكله أحلى + فلاش وارتداد
    const player = {
    ...
    function lockPointer(){
      ...
      // محاولة قفل الاتجاه (قد تفشل حسب المتصفح)
    ...
    // لمس: عصتين — مع منع السحب/الزووم/التحديد
    ...
    addEventListener('touchmove', e=>{
      for(const t of e.changedTouches){
        ...
        if(t.identifier===moveTouchId){
          ...
          player.yaw -= dx*0.01;
          player.pitch += dy*0.01; // عمودي صحيح
          player.pitch=Math.max(-1.2,Math.min(1.2,player.pitch));
    ...
    function updatePlayer(dt){
      ...
      // اهتزاز/ميلان السلاح (واقعية خفيفة)
    ...
    function updateEnemies(dt){
      for(const e of enemies){
        ...
        if(d<1.1 && e.userData.cooldown<=0){
          ...
          hpEl.textContent=player.hp; e.userData.cooldown=0.7;
        }
      }
      for(const b of bullets){
        for(const e of enemies){
          ...
          if(e.position.distanceToSquared(b.position)<0.7*0.7){
            ...
            if(e.userData.hp<=0){
              e.userData.dead=true; score+=10; scoreEl.textContent=score;
              const pl=new PointLight(0xff7766,2.5,6,2);
              pl.position.copy(e.position).add(new Vector3(0,1,0)); scene.add(pl);
    ...
    let recoil = 0, last=performance.now()/1000, running=false;
    function step(ms){
      if(!running) return;
    ...
    startBtn.onclick = ()=>{
      ...
      reset(); running=true; last=performance.now()/1000;
      requestAnimationFrame(step); lockPointer();
    };
